...
class TextConverter:
    def _get_text(self: ConverterProtocol, layer: Layer) -> Optional[svgwrite.text.Text]:
        ...
        if transform[1] != 0.0 and transform[2] != 0.0:
            text["transform"] = f"matrix{transform}"
        elif transform[0] != 1.0 and transform[3] != 1.0:
            text["transform"] = (
                f"translate({transform[4]},{transform[5]}) scale({transform[0]},{transform[3]})"
            )
        else:
            text["transform"] = f"translate({transform[4]},{transform[5]})"

        text["text-anchor"] = JUSTIFICATIONS.get(text_info["justification"], "start")
...
def _get_text_info(layer: Layer) -> Optional[dict[str, Any]]:
    ...
    direction = engine_data[b"EngineDict"][b"Rendered"][b"Shapes"][b"WritingDirection"]
    ...
    justification = paragraphs[0][b"ParagraphSheet"][b"Properties"].get(
        b"Justification", 0
    )
